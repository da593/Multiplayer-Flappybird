...
import { BoxCoordinates, Coordinates, GameState_I } from "./types";

export class GameStateProcessor {
    constructor() {    
    };

    detectCollision(birdCoords:BoxCoordinates, gapCoords:BoxCoordinates): boolean {
    ...
    calculateGapCoords(xPipeLoc: number, yGapLoc: number): BoxCoordinates {
    ...
    calculateBirdCoords(yBirdLoc: number): BoxCoordinates {
    ...
    calculateNewGapCoords(prevGapCoords: BoxCoordinates): BoxCoordinates {
        ...
        return this.calculateGapCoords(xPipeLoc,yGapLoc);
    }
    
    calculateNewBirdCoords(prevGapCoords: BoxCoordinates) {
        let yBirdLoc = prevGapCoords.topLeft.y;
        yBirdLoc = yBirdLoc + 1;
        return this.calculateBirdCoords(yBirdLoc);
    }
}